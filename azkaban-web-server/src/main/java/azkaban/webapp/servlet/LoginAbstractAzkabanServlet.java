...
public abstract class LoginAbstractAzkabanServlet extends AbstractAzkabanServlet {
    ...
    private Session createSession(final HttpServletRequest req)
        ...
        final String sessionId = req.getParameter("session.id");
        final String ip = getRealClientIpAddr(req);
        if(org.apache.commons.lang.StringUtils.isBlank(sessionId)){
          return createSession(username, password, ip);
        }else {
          return createSession(username, password, ip,sessionId);
        }
    }
    ...
    private Session createSession(final String username, final String password, final String ip, final String sessionId)
            throws UserManagerException {
        final UserManager manager = getApplication().getUserManager();
        final User user = manager.getUser(username, password);
